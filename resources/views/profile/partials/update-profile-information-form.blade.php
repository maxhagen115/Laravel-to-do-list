<section>
    ...
    <form id="profileForm" method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="profile_picture" name="profile_picture" type="file" id="file"
                accept="image/png, image/jpeg" class="mt-1 block w-full" />
        </div>
...
<script>
    document.getElementById('profileForm').addEventListener('submit', function(event) {
        ...
        axios.post('{{ route('profile.update') }}', formData)
            ...
            .catch(error => {
                toastr.error('Error updating profile. Please select another photo.');
